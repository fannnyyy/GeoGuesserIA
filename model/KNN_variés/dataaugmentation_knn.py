...
DEFAULT_CHECKPOINT_PATH = Path("checkpoints/knn_geo.pt")
DEFAULT_OUTPUT_PATH = Path("checkpoints/knn_geo_index.pt")
DEFAULT_SUMMARY_PATH = Path("checkpoints/knn_geo_summary.json")
DEFAULT_GEO_VIT_CKPT = Path("VIT/checkpoints/vit_geo_best.pt")
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return normalize_vec(features.float())


class GeoKNNRegressor:
    def __init__(self, temperature: float = 0.07, ref_chunk_size: int = 8192):
...
def build_feature_extractor(args: argparse.Namespace) -> FeatureExtractor:
    if args.feature_backbone == "geo_vit":
        checkpoint_path = Path(args.geo_vit_checkpoint)
        if not checkpoint_path.exists():
            raise FileNotFoundError(
                f"Geo ViT checkpoint not found: {checkpoint_path.resolve()}\n"
                "Use --geo-vit-checkpoint to point to a valid checkpoint, or switch to --feature-backbone resnet18/resnet50."
            )
        model = ViTGeo()
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        state_dict = checkpoint["model"] if isinstance(checkpoint, dict) and "model" in checkpoint else checkpoint
        model.load_state_dict(state_dict)
        feature_dim = model.net.heads.head.in_features
        model.net.heads = nn.Identity()
        return FeatureExtractor(model, feature_dim)

    if args.feature_backbone == "resnet18":
        weight_enum = models.ResNet18_Weights.DEFAULT if args.pretrained else None
        try:
            model = models.resnet18(weights=weight_enum)
        except Exception as exc:
            if not args.pretrained:
                raise
            print(f"[WARN] resnet18 pretrained weights unavailable ({exc}). Falling back to random initialization.")
            model = models.resnet18(weights=None)
        feature_dim = model.fc.in_features
        model.fc = nn.Identity()
        return FeatureExtractor(model, feature_dim)

    if args.feature_backbone == "resnet50":
        weight_enum = models.ResNet50_Weights.DEFAULT if args.pretrained else None
        try:
            model = models.resnet50(weights=weight_enum)
        except Exception as exc:
            if not args.pretrained:
                raise
            print(f"[WARN] resnet50 pretrained weights unavailable ({exc}). Falling back to random initialization.")
            model = models.resnet50(weights=None)
        feature_dim = model.fc.in_features
        model.fc = nn.Identity()
        return FeatureExtractor(model, feature_dim)

    raise ValueError(f"unsupported feature backbone: {args.feature_backbone}")
...
def checkpoint_run_config(args: argparse.Namespace) -> dict[str, object]:
    return {
        ...
        "geo_vit_checkpoint": str(args.geo_vit_checkpoint),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an augmented KNN geolocation model from image similarity and predict geographic coordinates as a unit vector."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="geo_vit",
        choices=["geo_vit", "resnet18", "resnet50"],
        help="Backbone used to extract image embeddings before the KNN search.",
    )
    parser.add_argument("--geo-vit-checkpoint", type=Path, default=DEFAULT_GEO_VIT_CKPT)
    ...
    parser.add_argument("--num-augmented-views", type=int, default=3)
    ...
    parser.add_argument("--country-penalty-multiplier", type=float, default=5.0)
    parser.add_argument("--k-values", type=parse_k_values, default=parse_k_values("1,3,5,10,20"))
...
def main() -> None:
    ...
    clean_train_loader = build_loader(
    ...
    feature_extractor = build_feature_extractor(args).to(device)
    feature_extractor.eval()
    print(f"Embedding backbone: {args.feature_backbone}")
    ...
    print(f"Saved KNN index to: {args.output_path.resolve()}")
    print(f"Saved summary to: {args.summary_path.resolve()}")
